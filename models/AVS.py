...
class AVS(nn.Module):
    ...
    def forward(self, frames, flengths, captions, lengths, plengths):
        video_features = self.encoder(frames, flengths)
        outputs, pos_outputs = self.decoder(video_features, flengths, captions, lengths, plengths)
...
class DecoderRNN(nn.Module):
    def __init__(self, args):
        ...
        self.lstm = nn.LSTM(self.embed_size + self.birnn * self.hidden_size, self.hidden_size, self.num_layers,
    ...
    def forward(self, video_features, flengths, captions, lengths, plengths):
        ...
        for i in range(step_size):
            ...
            c_t, beta = self.tem_atten(hidden_output, video_features, context_mask)
            ...
            inp = torch.cat((embeddings[:, i, :], gated_c_t), 1).unsqueeze(1)
    ...
    def sample(self, video_features, flengths, max_len=30, state=None):
        ...
        for i in range(max_len + 1):  # maximum sampling length
            ...
            inp = torch.cat((inputs, gated_c_t), 1).unsqueeze(1)
            # inp = self.input_proj(inp)
            hidden_output, state = self.lstm(inp, state)
            hidden_output = hidden_output.squeeze(1)
    ...
    def beam_search(self, video_features, flengths, max_len=20, beam_size=5):
        ...
        for i in range(1, max_len):
            ...
            for j in range(beam_size):
                ...
                c_t, beta = self.tem_atten(prev_hidden[j], video_features, context_mask)
