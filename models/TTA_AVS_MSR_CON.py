...
class TTA_AVS(nn.Module):
    def __init__(self, args):
        super(TTA_AVS, self).__init__()
        self.args = args
    ...
    def forward(self, frames, flengths, captions, lengths, tags, taglengths, plengths):
        video_features = self.encoder(frames, flengths)
        outputs, pos_outputs = self.decoder(video_features, flengths, captions, lengths, tags, taglengths, plengths)
    ...
    def sample_rl(self, frames, flengths, tags, tlengths, sampling='multinomial'):
        video_features = self.encoder.forward(frames, flengths)
        predicted_target, outputs, pos_outputs = self.decoder.rl_sample(video_features, flengths, tags, tlengths, sampling=sampling)
        return predicted_target, outputs, pos_outputs
...
class DecoderRNN(nn.Module):
    def __init__(self, args):
        ...
        self.text_atten = Attention(args, self.embed_size, self.hidden_size)
        ...
        self.pos1 = nn.Linear(self.hidden_size, self.hidden_size)

        self.softmax = nn.Softmax(dim=1)

        self.weight_pos = nn.Linear(self.hidden_size, 16)

        # self.pos2 = nn.Linear(16, 16)

        self.pos3 = nn.Linear(16, self.hidden_size)

        self.gate1x = nn.Linear(self.hidden_size * 2 + self.birnn * self.hidden_size,
                                self.hidden_size * 2 + self.birnn * self.hidden_size)

        self.gate2x = nn.Linear(self.hidden_size * 2 + self.birnn * self.hidden_size,
                                self.birnn * self.hidden_size)
        self.norm = nn.LayerNorm(self.birnn*self.hidden_size, eps=1e-6)
    ...
    def init_weights(self):
        ...
        self.pos1.weight.data.uniform_(-0.08, 0.08)
        self.pos1.bias.data.fill_(0)
        # self.pos2.weight.data.uniform_(-0.08, 0.08)
        # self.pos2.bias.data.fill_(0)
        self.pos3.weight.data.uniform_(-0.08, 0.08)
        self.pos3.bias.data.fill_(0)
        self.weight_pos.weight.data.uniform_(-0.08, 0.08)
        self.weight_pos.bias.data.fill_(0)
        self.gate1x.weight.data.uniform_(-0.08, 0.08)
        self.gate1x.bias.data.fill_(0)
        self.gate2x.weight.data.uniform_(-0.08, 0.08)
        self.gate2x.bias.data.fill_(0)

    def forward(self, video_features, flengths, captions, lengths, tags, tlengths, plengths):
        ...
        for i in range(step_size):
            ...
            vpos = self.pos1(hidden_output)
            vpos = torch.relu(self.dropout(vpos))
            dpos = self.weight_pos(vpos)
            dpos = self.dropout(dpos)
            pos_weights = self.softmax(dpos)
            ppos = self.pos3(pos_weights)
            # ppos = torch.relu(self.dropout(self.pos2(pos_weights)))
            # ppos = self.pos3(ppos)

            gate_input = torch.cat([hidden_output, ppos, c_t], -1)
            gate = torch.relu(self.dropout(self.gate1x(gate_input)))
            gate = torch.sigmoid(self.gate2x(gate))
            # gate = self.dropout(gate)

            gated_c_t = self.norm(gate * c_t + c_t)

            inp = torch.cat((embeddings[:, i, :], t_t, gated_c_t), 1).unsqueeze(1)
        ...
        pos_outputs = torch.transpose(torch.stack(pos_outputs), 0,
                                      1)
    ...
    def sample(self, video_features, flengths, tags, tlengths, max_len=30, state=None):
        ...
        for i in range(max_len + 1):  # maximum sampling length
            ...
            inp = torch.cat((inputs, t_t,gated_c_t), 1).unsqueeze(1)
            # inp = self.input_proj(inp)
            hidden_output, state = self.lstm(inp, state)
            hidden_output = hidden_output.squeeze(1)
    ...
    def beam_search(self, video_features, flengths, tags, tlengths, max_len=20, beam_size=5):
        ...
        for i in range(1, max_len):
            ...
            for j in range(beam_size):
                ...
                gate_input = torch.cat([prev_hidden[j], ppos, c_t], -1)
                gate = torch.relu(self.dropout(self.gate1x(gate_input)))
                gate = torch.sigmoid(self.gate2x(gate))
    ...
    def rl_sample(self, video_features, flengths, tags, tlengths, max_len=30, sampling='multinomial'):
        sampled_ids = []
        state = None
        outputs = []
        pos_outputs = []
        batch_size, max_enc_steps, _ = video_features.shape
        tag_features = self.tag_embed(tags)
        max_tag_steps = tag_features.shape[1]
        tag_mask = rnn_mask(tlengths, max_tag_steps)
        context_mask = rnn_mask(flengths, max_enc_steps)
        hidden_output = Variable(torch.FloatTensor(batch_size, self.hidden_size).zero_()).cuda()
        inputs = self.embed(Variable(torch.LongTensor(np.ones([batch_size, 1]))).cuda()).squeeze(1)
        ...
        pos_outputs = torch.transpose(torch.stack(pos_outputs), 0, 1)
        return sampled_ids.squeeze(), outputs, pos_outputs
